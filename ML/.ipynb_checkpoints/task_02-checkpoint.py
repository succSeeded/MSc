...
class KNN:
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        class_counts = np.count_nonzero((np.ndarray.flatten(d_sorted[:,:,1]) == classes_stacked).reshape(d_sorted.shape[0], d_sorted.shape[1],self.class_labels.shape[0]), axis=2)

        y_preds = np.take_along_axis(classes_stacked[:,:,0], np.argmax(class_counts, axis=0, keepdims=True), axis=0).flatten()
